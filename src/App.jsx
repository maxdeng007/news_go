...
function ThemeToggle({ isNewspaper, onToggle }) {
  return (
    <button 
      className="theme-toggle"
      onClick={onToggle}
      aria-label={isNewspaper ? 'Switch to dark theme' : 'Switch to newspaper theme'}
    >
      {isNewspaper ? '🌙 暗色主题' : '📰 日报纸风格'}
    </button>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-primary)]">
      <ThemeToggle 
        ...
        onToggle={() => setTheme(isNewspaper ? 'dark' : 'newspaper')} 
      />
      
      {isNewspaper && <NewsletterHeader />}
      <XiaoNuoya />
      {isNewspaper ? null : <DarkMeshGradient />}
      <Hero />
